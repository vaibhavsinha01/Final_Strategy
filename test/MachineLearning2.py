...
data = yf.download('SBIN.NS',start=startdate,end=enddate,interval='15m')
...
def feature_engineering(data):
    ...
    data['slope7'] = np.nan
    data['slope80'] = np.nan
    data['slope160'] = np.nan
    
    for i in range(7,len(data)):
        data['slope7'].iloc[i]=(data['Close'].iloc[i]-data['Close'].iloc[i-7])/7
    
    for i in range(80,len(data)):
        data['slope80'].iloc[i]=(data['Close'].iloc[i]-data['Close'].iloc[i-80])/80

    for i in range(160,len(data)):
        data['slope80'].iloc[i]=(data['Close'].iloc[i]-data['Close'].iloc[i-160])/160
