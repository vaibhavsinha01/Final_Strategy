...
class StrategyName:
    ...
    def Functions(self):
        ...
        self.data['Trend_prediction'] = np.where(self.data['DonchianChange'] > 0, 2, np.where(self.data['DonchianChange'] < 0, 0, 1))
        
        self.data = self.data.dropna()
        return self.data
    
    def Training(self):
        self.split = len(self.data) * 0.9
        # reg = GradientBoostingRegressor(n_estimators=300, learning_rate=0.3, max_depth=15, random_state=123)
        reg = DecisionTreeRegressor()
        x_train = self.data[['DonchianChange', 'ADX', 'Momentum']].iloc[:int(self.split)]
        y_train = self.data['Trend_prediction'].iloc[:int(self.split)]
        ...
        self.data['predict'] = reg.predict(self.data[['DonchianChange', 'ADX', 'Momentum']])
    ...
    def Signal(self):
        ...
        for i in range(int(self.split), int(len(self.data))):
            if (((self.data['sma160'].iloc[i] - self.data['sma160'].iloc[i - 20]) / 20) > 0) and (
            ...
            elif (((self.data['sma160'].iloc[i] - self.data['sma160'].iloc[i - 20]) / 20) < 0) and (
                    ((self.data['ADX'].iloc[i] - self.data['ADX'].iloc[i - 20]) / 20) < 0) and (
...
def main():
    for stock in stocks:
        ...
        session.FetchData(ticker)
        session.Functions()
        session.Training()  
        session.Signal()
        session.LogicPl()
            
        session.data.to_csv(os.path.join('data', f'file_{ticker}.csv'))
        print(session.data)
